...
function App() {
  ...
  const sealDate = async () => {
    const endpoint = import.meta.env.VITE_TELEGRAM_ENDPOINT || 'https://date-for-me-telegram.phatho0317.workers.dev/hen'
    if (endpoint) { try { const response = await fetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ date: formattedDate, time: formattedTime, plan: chosenPlan, place }) }); setTelegramSent(response.ok) } catch { setTelegramSent(false) } }
    setScreen(4)
  }
  ...
  return <main className="app-shell">
    ...
    <AnimatePresence mode="wait">
      {screen === 1 && <motion.section key="ask" {...pageMotion} transition={{ duration: .45 }} className="card ask-card">
        ...
        <p className="support-copy">Không cần áp lực đâu. Chỉ là một buổi hẹn nhỏ thui.</p>
      ...
      {screen === 2 && <motion.section key="yay" {...pageMotion} transition={{ duration: .45 }} className="card celebration-card"><Confetti />
        <div className="party-mark"><Heart size={28} fill="currentColor" /></div><p className="eyebrow">VUI QUÁ ĐIIIII</p><h1>Yeee — mình<br />có hẹn rồi.</h1><p className="lead">Anh biết em muốn đi chơi với anh mà :))))))</p>
      ...
      {screen === 3 && <motion.section key="plan" {...pageMotion} transition={{ duration: .45 }} className="card plan-card"><p className="eyebrow">MỜI BÀ CHỌN LỊCH TRÌNH MONG MUỐN</p><h1>Chọn ngày,<br />giờ và kế hoạch.</h1><p className="lead small-lead">Mình cùng chọn điều thật vui nhé.</p>
        ...
        {plan === 'Khác' && <label className="extra-field"><span>EM MUỐN LÀM GÌ?</span><input type="text" placeholder="Ví dụ: đi triển lãm, đi dạo..." value={customPlan} onChange={e => setCustomPlan(e.target.value)} /></label>}
        ...
        <button disabled={!canSeal} className="button primary full-button seal" onClick={sealDate}>Viết thư hẹn em <Mail size={16} /></button>
      </motion.section>}
    ...
    <AnimatePresence>{telegramSent && <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 12 }} className="toast telegram-toast"><Check size={16} /> Đã báo tin qua Telegram</motion.div>}</AnimatePresence>
  </main>
}
